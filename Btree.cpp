...
{
	int i;	
	if(x->child[0]==NULL) // if node has no child
	{						
		for(i=0;i<MAX;i++) // searching within the keys of the node
			if(key==x->ele[i])
				return NULL;	// sending NULL if found and as an indicator not to 
                // insert same value as
                // it already exists
		return x;   // sending the node otherwise, to return to ancestor
	}
	for(i=0;i<MAX;i++) // if the node does have children
	{
		if(x->ele[i] >= key)
